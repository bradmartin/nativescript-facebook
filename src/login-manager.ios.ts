...
export function _registerLoginCallback(callback: Function) {
	onLoginCallback = function (
		...
		if (result.token) {
			const token = result.token.tokenString;
...
export function requestReadPermissions(
	...
	_registerLoginCallback(callback);
	loginManager.logInWithPermissionsFromViewControllerHandler(
		permissions,
		undefined,
		onLoginCallback
	);
...
export function getCurrentAccessToken() {
	const sdkAccessToken = FBSDKAccessToken.currentAccessToken;
