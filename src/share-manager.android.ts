...
function attachAdditionalContent(
	content: any,
	addition?: ShareAdditionContent
) {
	if (addition) {
		if (addition.hashtag) {
...
export function createShareLinksContent(
	link: string,
	quote?: string,
	addition?: ShareAdditionContent
) {
	const content: com.facebook.share.model.ShareLinkContent.Builder = new com.facebook.share.model.ShareLinkContent.Builder().setContentUrl(
...
export function createSharePhotosContent(
	...
	const content = new com.facebook.share.model.SharePhotoContent.Builder();
...
export function createShareMessageGenericTemplateContent(
	contentConfig: MessageGenericTemplateContent
) {
	const elementConfig = contentConfig.element;
	const elementBuilder = new com.facebook.share.model.ShareMessengerGenericTemplateElement.Builder()
	...
	if (elementConfig.defaultAction) {
		elementBuilder.setDefaultAction(
			createMessageActionButton(elementConfig.defaultAction)
		);
	}
	...
	if (contentConfig.hasOwnProperty('imageAspectRatio')) {
		if (
			contentConfig.imageAspectRatio ===
			MessageGenericTemplateImageAspectRatio.Horizontal
		) {
			contentBuilder.setImageAspectRatio(
...
export function createShareMessageMediaTemplateContent(
	contentConfig: MessageMediaTemplateContent
) {
	...
	if (contentConfig.mediaUrl) {
		...
		throw new Error(
			'To use MediaTemplateContent, you have to provide either mediaUrl or attachmentID, see https://developers.facebook.com/docs/sharing/messenger#share-types for more detail'
		);
	}
	...
	if (contentConfig.pageID) {
		...
		throw new Error(
			'To use MediaTemplateContent, you have to provide a pageId, see https://developers.facebook.com/docs/sharing/messenger#app-page-id for more detail'
		);
	}

	if (contentConfig.mediaType === MessageMediaTemplateMediaType.Video) {
		contentBuilder.setMediaType(
			com.facebook.share.model.ShareMessengerMediaTemplateContent.MediaType
				.VIDEO
		);
	} else {
		contentBuilder.setMediaType(
...
export function showShareDialog(
	content: any,
	callback?: ShareCallbackFunction
) {
	const dialog = new com.facebook.share.widget.ShareDialog(
...
export function showMessageDialog(
	...
	const dialog = new com.facebook.share.widget.MessageDialog(
		Application.android.startActivity || Application.android.foregroundActivity
	);
	if (callback) {
		registerShareCallback(dialog, callback);
...
function registerShareCallback(
	dialog:
		| com.facebook.share.widget.ShareDialog
		| com.facebook.share.widget.MessageDialog,
	callback?: ShareCallbackFunction
) {
	const callbackManager = com.facebook.CallbackManager.Factory.create();
	dialog.registerCallback(
		callbackManager,
		new com.facebook.FacebookCallback<com.facebook.share.Sharer.Result>({
