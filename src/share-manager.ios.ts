...
export function createSharePhotosContent(
	...
	if (typeof images[0] === 'string') {
		nativeImages = (images as string[]).map(each => {
			return FBSDKSharePhoto.photoWithImageURLUserGenerated(
		...
		nativeImages = (images as ImageSource[]).map(each => {
			return FBSDKSharePhoto.photoWithImageUserGenerated(
				each.ios,
				userGenerated
			);
		});
...
export function createShareMessageGenericTemplateContent(
	...
	if (elementConfig.defaultAction) {
		element.defaultAction = createMessageActionButton(
			elementConfig.defaultAction
		);
	}
	...
	if (contentConfig.hasOwnProperty('imageAspectRatio')) {
		if (
			...
			content.imageAspectRatio =
				FBSDKShareMessengerGenericTemplateImageAspectRatio.Horizontal;
		} else {
			content.imageAspectRatio =
				FBSDKShareMessengerGenericTemplateImageAspectRatio.Square;
		}
	}
	return content;
}
...
export function createShareMessageMediaTemplateContent(
	...
	if (contentConfig.mediaUrl) {
		content = FBSDKShareMessengerMediaTemplateContent.alloc().initWithMediaURL(
			NSURL.URLWithString(contentConfig.mediaUrl)
		);
	} else if (contentConfig.attachmentID) {
		content = FBSDKShareMessengerMediaTemplateContent.alloc().initWithAttachmentID(
			contentConfig.attachmentID
		);
	} else {
		throw new Error(
...
export function showShareDialog(
	...
	FBSDKShareDialog.showFromViewControllerWithContentDelegate(
		currentViewController(),
		content,
		getCallbackDelegate(callback)
	);
}
...
export function showMessageDialog(
	...
	FBSDKMessageDialog.showWithContentDelegate(
		content,
		getCallbackDelegate(callback)
	);
}
...
class SharingDelegate extends NSObject implements FBSDKSharingDelegate {
	...
	static new(): SharingDelegate {
		if (
			SharingDelegate.ObjCProtocols.length === 0 &&
			typeof FBSDKSharingDelegate !== 'undefined'
		) {
